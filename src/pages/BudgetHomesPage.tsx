...
export function BudgetHomesPage() {
  ...
  const budgetProps = PROPERTIES.filter(p => (p.priceNum || 0) <= 5000000);

  return (
    <PageWrapper>
      <div className="bg-yellow-500/10 border border-warning/20 rounded-[2.5rem] p-12 mb-16 flex flex-col lg:flex-row gap-12 items-center">
        <div className="flex-1">
          <span className="bg-yellow-500/20 text-yellow-500 px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-widest mb-6 inline-block">
            Affordable Housing
          </span>
          ...
          <div className="flex gap-4">
            <button className="bg-yellow-500 hover:bg-yellow-500 text-white px-6 py-3 rounded-xl font-bold transition-colors shadow-lg shadow-amber-500/20">
              View Properties
            </button>
            <button className="bg-white text-yellow-500 border border-warning/40 hover:bg-yellow-500/20 px-6 py-3 rounded-xl font-bold transition-colors flex items-center gap-2">
        ...
        <div className="flex-1 w-full grid grid-cols-2 gap-4">
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 text-center">
            ...
            <h4 className="font-bold text-gray-900 mb-1">Low Down Payment</h4>
            <p className="text-xs text-gray-500">Pay as little as 10% upfront to book your home.</p>
          </div>
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 text-center">
            <Percent size={32} className="text-indigo-600 mx-auto mb-3" />
            <h4 className="font-bold text-gray-900 mb-1">PMAY Subsidy</h4>
