...
export function InsightsPage() {
  ...
  return (
    <PageWrapper>
      ...
      <div className="bg-gradient-to-br from-muted/10 to-secondary/10 rounded-3xl p-10 md:p-16 mb-16 text-center border border-gray-200 shadow-sm relative overflow-hidden">
        <div className="absolute top-0 right-0 w-64 h-64 bg-indigo-600/20 rounded-full blur-3xl -translate-y-1/2 translate-x-1/4 opacity-50" />
        <div className="relative z-10 flex flex-col items-center justify-center">
          <div className="w-16 h-16 bg-[#1A1A1A] rounded-2xl flex items-center justify-center text-pink-600 mb-6 shadow-lg">
            <Lightbulb size={32} />
          </div>
          <h1 className="text-3xl md:text-5xl font-black font-['Poppins'] text-gray-900 mb-4">
          ...
          <div className="flex w-full max-w-2xl bg-white rounded-xl shadow-md p-2 items-center border border-gray-200">
            <Search size={20} className="text-gray-500 ml-4 mr-2" />
            <input 
              type="text" 
              placeholder="Enter a city, locality or society" 
              className="flex-1 bg-transparent border-none outline-none py-3 text-sm"
            />
            <button className="bg-pink-600 hover:bg-[#E8355F] text-white px-6 py-3 rounded-lg font-bold text-sm transition-colors">
              Search Insights
            </button>
          </div>
        </div>
      ...
      <div className="mb-16">
        <div className="flex justify-between items-end mb-6">
          <div>
            <h2 className="text-2xl font-bold font-['Poppins'] text-gray-900">Discover in Pune</h2>
            <p className="text-gray-500 text-sm mt-1">Localities and Societies in Pune</p>
          </div>
          <button className="text-pink-600 text-sm font-bold border border-pink-600 px-4 py-2 rounded-lg hover:bg-pink-600/5 transition-colors">
        ...
        <div className="flex overflow-x-auto gap-6 pb-4 snap-x hide-scrollbar">
          {DISCOVER_CAROUSEL.map(item => (
            <div key={item.id} className="min-w-[240px] md:min-w-[280px] h-48 rounded-2xl relative overflow-hidden group cursor-pointer snap-start flex-shrink-0 shadow-sm border border-gray-200">
              <img 
      ...
      <div className="mb-16 bg-white border border-gray-200 shadow-sm rounded-3xl p-6 md:p-10">
        <div className="mb-8">
          <h2 className="text-2xl font-bold font-['Poppins'] text-gray-900">Top Gainers</h2>
        ...
        <div className="flex gap-6 border-b border-gray-200 mb-6 overflow-x-auto hide-scrollbar">
          {Object.keys(TOP_GAINERS_CITIES).map(city => (
            <button 
              ...
              className={`pb-4 text-sm font-bold whitespace-nowrap border-b-2 transition-colors ${activeGainerTab === city ? 'border-pink-600 text-pink-600' : 'border-transparent text-gray-500 hover:text-gray-900'}`}
            >
              {city}
            </button>
          ))}
            ...
            <tbody className="divide-y divide-gray-50">
              {TOP_GAINERS_CITIES[activeGainerTab as keyof typeof TOP_GAINERS_CITIES]?.map((loc, idx) => (
                <tr key={idx} className="hover:bg-gray-50 transition-colors group cursor-pointer">
                  ...
                  <td className="py-5 px-6">
                    <p className="text-xs text-gray-500 mb-1">Rate on Platform</p>
                    <p className="font-bold font-['Poppins'] text-gray-900">{loc.priceRange}</p>
                  </td>
                  <td className="py-5 px-6">
                    <div className="flex flex-col items-start gap-1">
                      ...
                      <div className="w-24 h-4 rounded bg-gradient-to-t from-success/20 to-transparent border-t-2 border-success/80 opacity-70" />
                    </div>
                  </td>
                </tr>
              ))}
      ...
      <div className="mb-16">
        <div className="flex items-center gap-2 mb-2">
          <h2 className="text-2xl font-bold font-['Poppins'] text-gray-900">Check Transaction Prices</h2>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          <div className="lg:w-1/3 bg-[#1A1A1A] rounded-3xl p-8 text-white relative overflow-hidden shadow-lg">
            ...
            <div className="relative z-10">
              ...
              <button className="bg-pink-600 hover:bg-[#E8355F] w-full py-3.5 rounded-xl font-bold transition-colors">
                Search Transactions
              </button>
            </div>
          </div>
          
          <div className="lg:w-2/3 bg-white rounded-3xl border border-gray-200 shadow-sm overflow-hidden">
            <div className="flex overflow-x-auto hide-scrollbar border-b border-gray-200">
              {['Pune', 'Bangalore', 'Chennai', 'Mumbai'].map(city => (
                <button key={city} className={`px-6 py-4 text-sm font-bold whitespace-nowrap border-b-2 ${city === 'Pune' ? 'border-pink-600 text-pink-600' : 'border-transparent text-gray-500'}`}>
                  {city}
                </button>
              ))}
            </div>
            <div className="p-2">
              {TRANSACTIONS.map((txn, idx) => (
                <div key={idx} className="flex items-center justify-between p-4 hover:bg-gray-50 rounded-xl cursor-pointer group">
                  <div>
                    <p className="font-bold text-gray-900">{txn.locality}</p>
                    <p className="text-xs text-gray-500 mt-1">{txn.count} properties sold</p>
                  </div>
                  <div className="text-right">
                    <p className="font-bold text-gray-900 font-['Poppins']">{txn.avgRate}</p>
                    <p className="text-xs text-pink-600 group-hover:underline mt-1 font-semibold flex items-center gap-1 justify-end">
      ...
      <div className="mb-20">
        <div className="flex items-end justify-between mb-8">
          <div>
            <h2 className="text-2xl font-bold font-['Poppins'] text-gray-900">Ratings & Reviews</h2>
            <p className="text-gray-500 mt-1 text-sm">See what residents and owners have to say</p>
          </div>
          <button className="text-sm font-bold text-pink-600 hover:underline">
            Write a Review
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {LOCALITY_REVIEWS.map((review) => (
            <div key={review.id} className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm flex flex-col justify-between">
              <div>
                <div className="flex items-center justify-between mb-4">
                  <p className="font-bold text-gray-900">{review.locality}</p>
                  <div className="bg-pink-600/10 text-pink-600 px-2.5 py-1 rounded-lg text-xs font-bold flex items-center gap-1">
                    <Star size={12} className="fill-[#FF3F6C]" /> {review.rating}
                  </div>
                </div>
                <p className="text-gray-500 text-sm leading-relaxed mb-6 italic">"{review.text}"</p>
              </div>
              <div className="flex items-center justify-between border-t border-gray-200 pt-4 mt-4">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-full bg-gray-50 flex items-center justify-center text-xs font-bold text-gray-900">
