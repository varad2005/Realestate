...
export function PlotsPage() {
  ...
  return (
    <PageWrapper>
      <div className="bg-emerald-500 rounded-[2.5rem] p-12 text-white mb-16 relative overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-success via-success/80 to-transparent" />
        
        <div className="relative z-10 max-w-2xl">
          <span className="bg-white/20 text-emerald-500/20 px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-widest backdrop-blur-sm mb-6 inline-block border border-white/30">
            Land Investments
          </span>
          <h1 className="text-4xl md:text-5xl font-black font-['Poppins'] leading-tight mb-6">
            Invest in Land, Build Your Future
          </h1>
          <p className="text-emerald-500/20 text-lg mb-8">
            Discover agricultural, residential, and commercial plots with high appreciation potential.
          </p>
          
          <div className="flex flex-wrap gap-4">
            <button className="bg-white text-emerald-500 px-6 py-3 rounded-xl font-bold hover:bg-emerald-500/10 transition-colors shadow-lg">
              View Residential Plots
            </button>
            <button className="bg-emerald-500 text-white border border-success px-6 py-3 rounded-xl font-bold hover:bg-emerald-500 transition-colors shadow-lg">
              Explore Farm Lands
            </button>
          </div>
        </div>
        ...
        {plotProps.length > 0 ? plotProps.map(property => (
          ...
          [1,2,3,4].map(i => (
             <div key={i} className="bg-white rounded-2xl p-4 border border-gray-200 shadow-sm">
                ...
                <h3 className="font-bold text-gray-900 mb-1">Premium Residential Plot</h3>
                <p className="text-sm text-gray-500 mb-2">Hinjewadi Phase 3, Pune</p>
                <p className="font-bold text-pink-600">₹45 L</p>
             </div>
          ))
        )}
      </div>
