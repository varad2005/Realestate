      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
        ...
        <div className="flex items-center gap-4 text-sm font-medium">
          <div className="bg-white px-4 py-2 rounded-lg shadow-sm border border-gray-100">
            Total Saved: <span className="text-[#FF3F6C] font-bold">{properties.length}</span>
          </div>
        </div>
      </div>
      ...
      {properties.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {properties.map(property => {
            ...
            return (
              <div key={property.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden hover:shadow-md transition-shadow group cursor-pointer" onClick={() => navigate(`/property/${property.id}`)}>
                <div className="relative h-48 overflow-hidden">
                  ...
                  <button 
                    ...
                    className="absolute top-3 right-3 w-8 h-8 bg-white/90 backdrop-blur-sm rounded-full flex items-center justify-center shadow-sm hover:scale-110 transition-transform text-[#FF3F6C]"
                  >
                ...
                <div className="p-5">
                  ...
                  <div className="grid grid-cols-1 gap-2 border-t border-gray-100 pt-4">
                    <button 
                      ...
                      className="w-full flex items-center justify-center gap-2 py-2 text-gray-700 hover:text-[#FF3F6C] hover:bg-pink-50 rounded-lg transition-colors font-medium text-sm"
                    >
                      <Eye size={18} />
                      View Details
                    </button>
