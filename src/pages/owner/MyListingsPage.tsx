      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
        ...
        <div className="flex items-center gap-4 text-sm font-medium">
          <div className="bg-white px-4 py-2 rounded-lg shadow-sm border border-gray-200">
            Total: <span className="text-pink-600 font-bold">{properties.length}</span>
          </div>
          <div className="bg-white px-4 py-2 rounded-lg shadow-sm border border-gray-200">
            Approved: <span className="text-emerald-500 font-bold">{approvedCount}</span>
          </div>
          <div className="bg-white px-4 py-2 rounded-lg shadow-sm border border-gray-200">
            Pending: <span className="text-yellow-500 font-bold">{pendingCount}</span>
          </div>
        </div>
      </div>
      ...
      {properties.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {properties.map(property => {
            ...
            return (
              <div key={property.id} className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden hover:shadow-md transition-shadow group">
                <div className="relative h-48 overflow-hidden">
                  ...
                  <div className="absolute top-3 right-3 flex flex-col gap-2">
                    ...
                    {tourCount > 0 && (
                      <span className="bg-purple-600 text-white px-2 py-0.5 text-[10px] font-bold rounded-full flex items-center gap-1">
                ...
                <div className="p-5">
                  ...
                  {property.property_addon_orders && property.property_addon_orders.length > 0 && (
                    <div className="flex flex-wrap gap-1 mb-3">
                      {property.property_addon_orders.map((order: any) => order.addon && (
                        <span key={order.id} className="bg-purple-600/10 text-purple-600 border border-accent/40 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider">
                  ...
                  <button
                    ...
                    className={`w-full flex items-center justify-center gap-2 py-2 rounded-lg text-sm font-bold mb-3 transition-all ${
                      tourCount > 0
                        ? 'bg-purple-600/10 text-purple-600 border border-accent/40 hover:bg-purple-600/20'
                        : 'bg-gray-50 text-gray-500 border border-gray-200 hover:bg-gray-50 hover:text-purple-600'
                    }`}
                  >
                    <Globe size={15} />
                  ...
                  <div className="grid grid-cols-3 gap-2 border-t border-gray-200 pt-4">
                    <button 
                      ...
                      className="flex flex-col items-center justify-center gap-1 py-2 text-gray-500 hover:text-pink-600 hover:bg-pink-600/10 rounded-lg transition-colors"
                    >
                      <Eye size={18} />
                    ...
                    <button 
                      ...
                      className="flex flex-col items-center justify-center gap-1 py-2 text-gray-500 hover:text-indigo-600 hover:bg-indigo-600/10 rounded-lg transition-colors"
                    >
                      <Edit size={18} />
                    ...
                    <button 
                      ...
                      className="flex flex-col items-center justify-center gap-1 py-2 text-gray-500 hover:text-red-500 hover:bg-red-500/10 rounded-lg transition-colors"
                    >
                      <Trash2 size={18} />
          ...
          >
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs font-bold text-purple-600 uppercase tracking-wider">360° Virtual Tour</p>
                  <p className="text-sm font-semibold text-gray-900 truncate max-w-[220px]">{tourManager.propertyTitle}</p>
                </div>
              </div>
              <button onClick={closeTourManager} className="w-8 h-8 rounded-full hover:bg-gray-50 flex items-center justify-center text-gray-500 hover:text-gray-900 transition-colors">
                <X size={18} />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6">
              {tourManager.loading ? (
                ...
                <div className="text-center py-8 mb-4">
                  <Globe size={40} className="text-gray-500/50 mx-auto mb-3" />
                  <p className="text-gray-500 text-sm mb-2">No 360° scenes added yet.</p>
                  <p className="text-xs text-gray-500">Upload your first panorama below.</p>
                </div>
              ) : (
                <div className="space-y-3 mb-4">
                  {tourManager.scenes.map((scene) => (
                    <div key={scene.id} className={`flex gap-3 items-center rounded-xl p-3 border ${scene.is_default ? 'border-accent/40 bg-purple-600/10/40' : 'border-gray-200'}`}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {scene.is_default && (
                          <span className="text-[10px] font-bold text-purple-600 bg-purple-600/20 px-1.5 py-0.5 rounded">Default</span>
                        )}
                      </div>
                      <div className="flex items-center gap-1 shrink-0">
                        {!scene.is_default && (
                          <button
                            ...
                            className="w-7 h-7 rounded-full hover:bg-purple-600/20 text-gray-500 hover:text-purple-600 flex items-center justify-center transition-colors"
                          >
                            <Star size={14} />
                          </button>
                        )}
                        <button
                          ...
                          title="Delete scene"
                          className="w-7 h-7 rounded-full hover:bg-red-500/10 text-gray-500 hover:text-red-500 flex items-center justify-center transition-colors"
                        >
                          <Trash2 size={14} />
                        </button>
                      </div>
              ...
              {/* Upload new scene */}
              <div
                className="border-2 border-dashed border-accent/40 rounded-xl p-6 text-center cursor-pointer hover:bg-purple-600/10/30 transition-colors relative"
                onClick={() => document.getElementById('tour-manager-upload')?.click()}
              >
                {tourManager.uploading ? (
                  <div className="flex flex-col items-center gap-2">
                    <Loader2 size={24} className="animate-spin text-purple-600" />
                    <p className="text-sm font-medium text-purple-600">Uploading panorama…</p>
                  </div>
                ) : (
                  <>
                    <Upload size={22} className="text-purple-600 mx-auto mb-2" />
                    <p className="text-sm font-bold text-purple-600 mb-1">Add New 360° Scene</p>
                    <p className="text-xs text-gray-500">JPG, PNG, WebP • Max 50 MB</p>
                  </>
                )}
                <input
