  ...
  return (
    <PageWrapper>
      <div className="bg-[#1A1A1A] rounded-[2.5rem] p-12 text-white mb-16 relative overflow-hidden flex flex-col md:flex-row gap-12 items-center">
        <div className="absolute -left-20 -top-20 w-96 h-96 bg-purple-500/20 rounded-full blur-3xl" />
        <div className="relative z-10 flex-1">
          <span className="bg-purple-500/20 text-purple-300 px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-widest backdrop-blur-sm mb-6 inline-block border border-purple-400/30">
          ...
          <ul className="space-y-3 mb-8">
            {["Lower launch prices", "Better choice of units", "Flexible payment plans"].map((benefit, i) => (
              <li key={i} className="flex items-center gap-3 text-sm font-semibold text-gray-300">
                <CheckCircle2 size={18} className="text-purple-400" /> {benefit}
              </li>
            ))}
          </ul>
        </div>
        <div className="relative z-10 flex-1 w-full">
          <div className="relative rounded-2xl overflow-hidden shadow-2xl border border-white/10 group cursor-pointer">
            ...
            <div className="absolute bottom-6 left-6 right-6">
              <div className="bg-white/90 backdrop-blur-sm text-black px-3 py-1 rounded-full text-[10px] font-bold uppercase inline-block mb-3">Featured Project</div>
              <h3 className="text-2xl font-bold font-['Poppins'] mb-1">Lodha Panache</h3>
              <p className="text-gray-300 text-sm flex items-center gap-2"><Building size={14} /> Hinjewadi, Pune</p>
            </div>
          </div>
        </div>
      </div>
